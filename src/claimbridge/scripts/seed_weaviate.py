...
def seed_weaviate(weaviate_url: Optional[str] = None) -> dict:
    """
    ...
    Args:
        weaviate_url: URL to Weaviate instance. None (the default) means read
                      $WEAVIATE_URL, which docker-compose sets to
                      http://weaviate:8080. Hardcoding localhost here would
                      break when this runs inside the container.
...
def verify_seeding(weaviate_url: Optional[str] = None) -> dict:
    """
